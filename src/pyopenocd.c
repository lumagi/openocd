...
struct command_context *create_command_handler(Jim_Interp *interp)
{
	return command_init(openocd_startup_tcl, interp);
}

static struct command_context *setup_command_handler(struct command_context *cmd_ctx, Jim_Interp *interp)
{
	log_init();
...
int openocd_init(struct command_context *cmd_ctx, int argc, char **argv)
{
	int ret;

	setup_command_handler(cmd_ctx, NULL);
...
int openocd_deinit(struct command_context *cmd_ctx)
{
	int ret = ERROR_OK;
